...
# IDW function for rainfall

# Custom IDW function
idw_interpolation <- function(points, values, grid_points, power = 2, nmax = 10) {
  distances <- st_distance(grid_points, points)
  distances <- as.matrix(distances)
  
  # Vectorize the calculations where possible
  interpolated <- numeric(nrow(distances))
  for(i in 1:nrow(distances)) {
    d <- distances[i,]
    # Only use nmax nearest points
    nearest <- order(d)[1:min(nmax, length(d))]
    d <- d[nearest]
    v <- values[nearest]
    ...
    interpolated[i] <- sum(w * v)
  }
  return(interpolated)
}


# Modified plotting function for wind speed
plot_windspeed_idw <- function(data, year, month) {
  # Filter data
  filtered_data <- data %>%
    mutate(
      Year = year(date),
    ...
    filter(Year == year, 
           as.character(Month) == as.character(month))
  
  # Create simple grid (reduced resolution for speed)
  bbox <- st_bbox(filtered_data)
  grid_x <- seq(bbox[1], bbox[3], length.out = 30)
  grid_y <- seq(bbox[2], bbox[4], length.out = 30)
  grid_points <- expand.grid(x = grid_x, y = grid_y) %>%
    st_as_sf(coords = c("x", "y"), crs = st_crs(filtered_data))
  
  # Simple IDW calculation
  points_coords <- st_coordinates(filtered_data)
  grid_coords <- st_coordinates(grid_points)
  values <- filtered_data$`Mean Wind Speed (km/h)`
  
  # Calculate interpolated values
  interpolated <- sapply(1:nrow(grid_coords), function(i) {
    distances <- sqrt((grid_coords[i,1] - points_coords[,1])^2 + 
                        (grid_coords[i,2] - points_coords[,2])^2)
    weights <- 1/distances^2
    weights[is.infinite(weights)] <- 1e10
    sum(weights * values) / sum(weights)
  })
  
  # Add values to grid
  grid_points$value <- interpolated
  ...
  tm_shape(mpsz) +
    ...
    tm_dots(col = "value", 
            ...
            size = 0.1) +
    tm_layout(title = paste("Wind Speed IDW -", month, year)) +
    tm_view(set.zoom.limits = c(11, 14))
}

# Similar function for rainfall
plot_rainfall_idw <- function(data, year, month) {
  filtered_data <- data %>%
    mutate(
      Year = year(date),
      Month = month(date, label = TRUE, abbr = FALSE)
  ...
  points_coords <- st_coordinates(filtered_data)
  grid_coords <- st_coordinates(grid_points)
  values <- filtered_data$`Daily Rainfall Total (mm)`
  ...
  tm_shape(mpsz) +
    ...
    tm_dots(col = "value", 
            ...
            title = "Rainfall (mm)",
            size = 0.1) +
    tm_layout(title = paste("Rainfall IDW -", month, year)) +
    tm_view(set.zoom.limits = c(11, 14))
}

# And for temperature
plot_temperature_idw <- function(data, year, month) {
  filtered_data <- data %>%
    mutate(
      Year = year(date),
      Month = month(date, label = TRUE, abbr = FALSE)
  ...
  points_coords <- st_coordinates(filtered_data)
  grid_coords <- st_coordinates(grid_points)
  values <- filtered_data$`Mean Temperature (°C)`
  ...
  tm_shape(mpsz) +
    ...
    tm_dots(col = "value", 
            ...
            title = "Temperature (°C)",
            size = 0.1) +
    tm_layout(title = paste("Temperature IDW -", month, year)) +
    tm_view(set.zoom.limits = c(11, 14))
}
...
server <- function(input, output, session) {
  ...
  output$windspeed_idw_map <- renderTmap({
    ...
    withProgress(message = 'Creating wind speed map...', {
      plot_windspeed_idw(
        data = climate_windspeed3414,
        year = input$year,
        month = input$month
      )
    })
  ...
  output$rainfall_idw_map <- renderTmap({
    req(input$year, input$month)
    withProgress(message = 'Creating rainfall map...', {
      plot_rainfall_idw(
        data = climate_rainfall3414,
        year = input$year,
        month = input$month
      )
    })
  ...
  output$temperature_idw_map <- renderTmap({
    req(input$year, input$month)
    withProgress(message = 'Creating temperature map...', {
      plot_temperature_idw(
        data = climate_temperature3414,
        year = input$year,
        month = input$month
      )
    })
